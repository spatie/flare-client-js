...
- **IPC receiver.** Registers `ipcMain.handle('flare:report', handler)`. The
 ...
 `beforeSubmit` for scrubbing; see below.)
    ...
    **Hardening (implemented in 0.1.0).** The overlay is UNCONDITIONAL for the three
    config-derived fields: `receiveRendererReport` writes `service.stage` /
    `service.version` / `sourcemapVersionId` when main has them set, and DELETES any
    renderer-supplied value when main does not (`overlayOrDelete`). A renderer can
    therefore never supply these fields regardless of main's config state, so the
    "configure once, in main" guarantee holds even for an unconfigured field. Tested
    in `electronFlare.test.ts` (strip case). This matters most when an app opts into
    trusting remote/custom-protocol senders; under the default sender-trust policy
    only the app's own `file:`/localhost renderer is accepted.
