...
describe('ElectronFlare', () => {
    ...
    it('setUser writes user.* on the next main-origin report', async () => {
        ...
        expect(api.reports[0].attributes['user.id']).toBe('7');
        expect(api.reports[0].attributes['user.email']).toBe('u@x.io');
        expect(api.reports[0].attributes['enduser.id']).toBeUndefined();
        flare.setUser(null);
        await flare.report(new Error('no user'));
        expect(api.reports[1].attributes['user.id']).toBeUndefined();
    });

    it('forwarded reports get main config (stage/version/sourcemap) overlaid', async () => {
        const { flare, ipcMain, api } = makeFlare();
        ...
        expect(api.reports.length).toBe(1);
        expect(api.reports[0].attributes['service.stage']).toBe('production');
        expect(api.reports[0].attributes['service.version']).toBe('2.0.0');
        ...
        expect(api.reports[0].attributes['user.id']).toBe('99');
        expect(api.reports[0].attributes['user.email']).toBe('main@user.io');
    });
    ...
    it('strips renderer-supplied identity when main never called setUser', async () => {
        ...
        expect(api.reports[0].attributes['user.id']).toBeUndefined();
        expect(api.reports[0].attributes['user.email']).toBeUndefined();
    });
    ...
    it('strips renderer-supplied stage/version/sourcemap when main never configured them', async () => {
        ...
        expect(api.reports[0].attributes['service.stage']).toBeUndefined();
        expect(api.reports[0].attributes['service.version']).toBeUndefined();
        expect(api.reports[0].sourcemapVersionId).toBeUndefined();
    ...
    it('configureElectron ignores explicit undefined and preserves resolved defaults', async () => {
        const { flare, api, ipcMain } = makeFlare();
