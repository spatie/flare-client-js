...
// Resolves a router-reported location (a `URL` on visits, a relative string on pages) to a full href
...
// Background work fires the same `start` and `finish` a real visit does, so opening a root for one
// invents a navigation and ends the one that was live.
function isBackgroundVisit(visit: InertiaVisitLike | undefined): boolean {
    ...
    // `router.reload()` backs polling, deferred props and infinite scroll, always as `async: true` on
    // the current url — so an async visit on the same page is a refresh. Compares path, not url, since
    // infinite scroll only changes the query. A deliberate async visit to another page still opens a root.
    const { path } = locationOf(visit.url);
    return !!visit.async && path !== undefined && path === currentPath();
}

// `component` ('Products/Show') is Inertia's route identifier, and there's a small fixed set of them,
...
function install(router: InertiaRouterLike, track: TrackTeardown): void {
    ...
    // The component name of the page currently shown, per Inertia. A visit that ends without its own
    // page is named after the page it left the user on, instead of a raw url like /products/42/comments.
    ...
    on('start', (event: InertiaEventLike) => {
        ...
        if (inFlight) {
            // The successor of an interrupted visit. Re-point the root the first click opened instead
            // of opening a second one; the name stays url-sourced until the page arrives and settles it.
            inFlightPath = path;
    ...
    on('navigate', (event: InertiaEventLike) => {
        ...
        // Deliberately unguarded, unlike `success` and `finish`: a redirect (POST /login -> /dashboard)
        // has a page url that won't match inFlightPath, but it's the only event carrying that page's
        // component name. Matching the path would push redirects to the `finish` backstop, which has no
        // name and falls back to a raw url.
        if (inFlight) {
            settle(page);
            return;
        }
        ...
        // (forces replace) but InitialVisit.handle() fires it directly. Name the existing pageload
        // root rather than open a second one.
        if (!sawInitial) {
            sawInitial = true;
        ...
        // Back/forward fires navigate alone, with no start to open the root, so this does both. No
        // hold: there's no wait to suppress, and settling in the same tick would force-close it at
        // zero duration before a child span could attach.
        const { href, path } = locationOf(page?.url);
    ...
    on('success', (event: InertiaEventLike) => {
        // This is where a same-url visit and a `replace: true` visit settle: page.set() only fires
        // navigate when replace is false, and a visit landing on its own start url forces replace.
        if (!inFlight) {
            return;
        }
        const page = event?.detail?.page;
        ...
        // a redirect back to the user's page looks like a background poll of it. Not a leak — `finish`
        // compares the stable `visit.url` and settles through its backstop.
    ...
    const belongsToThisNavigation = (visit: InertiaVisitLike | undefined): boolean => {
        ...
        // Inertia only interrupts from inside `visit()`, right before sending the replacement, so the
        // successor `start` can settle this root. `cancelled` has no successor, so it deliberately
        // falls through and settles here.
    ...
    on('finish', (event: InertiaEventLike) => {
        ...
        // this the held root stays idle-suppressed until the 30s finalTimeout. Settles to the browser's
        // actual location, since the visit never reached its destination.
        if (!inFlight) {
            return;
        }
