...
// Replaces `source[name]` with `replacer(original)`, tagging the wrapper with a non-enumerable
// `__flare_original__` so the patch is idempotent and reversible. Ported from Sentry's `fill`,
// minus the prototype/own-property copying not needed for `fetch`.
