...
describe('formatFailureBanner', () => {
    ...
    test('puts the reason on its own `  Reason: <reason>` line (parsed by flare-xcode.sh)', () => {
        // flare-xcode.sh lifts this exact line into the Xcode warning via sed. The format is a contract
        // (two-space indent, `Reason: `, then the reason) — don't change it without updating that script.
