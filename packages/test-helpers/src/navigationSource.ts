...
// mid-test assertion failure that skipped its own `unregister()`. A fresh registration replaces a
// leaked one (last-wins), and unregistering that fresh handle clears the slot. Pass the real
// `registerNavigationSource` from `@flareapp/js/browser`, since this package cannot import it
// directly without a circular workspace dependency.
