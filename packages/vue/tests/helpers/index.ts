...
// Fake Vue `ComponentPublicInstance`. `name` sets `$options.__name` (the SFC-compiler name);
// `overrides.name` sets the legacy `$options.name` fallback, so both `getComponentName` paths can
// be tested independently. `parent` defaults to null, `props` defaults to `{}`.
